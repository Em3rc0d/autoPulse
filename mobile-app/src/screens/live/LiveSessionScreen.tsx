...
export default function LiveSessionScreen() {
  ...
  useEffect(() => {
    if (initialAdapterVoltage && cardsRef.current['CONTROL_MODULE_VOLTAGE']) {
      const match = String(initialAdapterVoltage).match(/(\d+(?:\.\d+)?)/);
      cardsRef.current['CONTROL_MODULE_VOLTAGE'].update(match ? Number(match[1]) : null, match ? 'VALID' : 'UNAVAILABLE');
    }
