    ...
    try {
      ...
      if (res0100.status !== 'SUCCESS_DECODED' && res0100.status !== 'SUCCESS_RAW' && res0100.status !== 'NO_DATA') {
        for (const protocol of ['ATSP6', 'ATSP7', 'ATSP8', 'ATSP9', 'ATSP3', 'ATSP4', 'ATSP5', 'ATSP1', 'ATSP2']) {
          ...
          if (res0100.status === 'SUCCESS_DECODED' || res0100.status === 'SUCCESS_RAW' || res0100.status === 'NO_DATA') {
            const resDp = await this.executeAndRecord(snapshot, 'ATDP', 3000, 'ELM_AT');
            this.rememberIdentity(snapshot, 'protocolName', resDp);
            const resDpn = await this.executeAndRecord(snapshot, 'ATDPN', 3000, 'ELM_AT');
            snapshot.protocol = resDpn.normalizedResponse?.normalizedText || resDp.normalizedResponse?.normalizedText || protocol;
            break;
          }
        }
      }
