...
const deadlinePolicy = {
  ...
  stageDeadlineMs: { CAPABILITY_DISCOVERY: 1500, DTC_CORE: 3000 },
...
describe('CHECK-MK5 DiagnosticScanPlanner', () => {
  it('builds a serial descriptor-only plan with deterministic evidence traceability', () => {
    ...
    expect(plan.requests.map(item => item.semanticId)).toEqual([
      'check.obd.mode03.stored-dtc', 'check.obd.mode07.pending-dtc', 'check.obd.mode0a.permanent-dtc',
    ]);
    ...
    expect(evidenceTraceForPlannedRequest(first)).toEqual(expect.objectContaining({
      planId: 'plan-1', descriptorId: first.descriptorId, semanticId: first.semanticId, parserContractId: first.parserContractId,
    }));
  ...
  it('preserves required severity when duplicate proposals are consolidated', () => {
    ...
    expect(unsafe.blockedProposals).toEqual([
      expect.objectContaining({ semanticId: 'check.obd.mode04.clear-dtc', required: true, reason: 'UNREGISTERED_DESCRIPTOR' }),
    ]);
  ...
  it('fails closed when a descriptor is registered but not parser-promoted for the active protocol', () => {
    const plan = build({ protocol: 'ISO_15765_CAN', proposals: [
      { semanticId: 'check.obd.mode03.stored-dtc', required: true },
      { semanticId: 'check.obd.mode07.pending-dtc', required: true },
    ] });
    expect(plan.status).toBe('BLOCKED');
  ...
  it('blocks when required baseline commands cannot fit in the plan budget', () => {
    const plan = build({ budget: { ...budget, maxCommands: 1 }, proposals: [
      { semanticId: 'check.obd.mode03.stored-dtc', required: true },
      { semanticId: 'check.obd.mode07.pending-dtc', required: true },
    ] });
    expect(plan.status).toBe('BLOCKED');
  ...
  it('trims only optional work when command capacity remains for required work', () => {
    const plan = build({ budget: { ...budget, maxCommands: 2 }, proposals: [
      { semanticId: 'check.obd.mode03.stored-dtc', required: true },
      { semanticId: 'check.obd.mode07.pending-dtc', required: true },
      { semanticId: 'check.obd.mode0a.permanent-dtc', required: false },
    ] });
  ...
  it('prevents any new request after cancellation, deadline, pacing or budget exhaustion', () => {
    ...
    const baseContext = {
      planRequestId: request.planRequestId, scanStartedAt: 0, stageStartedAt: 100, now: 200, cancelRequested: false,
  ...
  it('rejects a request id that is not part of the immutable plan', () => {
    ...
    expect(evaluatePlannedRequestGate(plan, {
      planRequestId: 'forged-request', scanStartedAt: 0, stageStartedAt: 100, now: 200, cancelRequested: false,
      budgetUsage: { commandsIssued: 0, responseBytes: 0 },
