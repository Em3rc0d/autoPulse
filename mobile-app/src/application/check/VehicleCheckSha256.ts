import { encodeUtf8 } from '../../infrastructure/runtime/TextEncodingPolyfill';
...
/** Hermes-safe SHA-256 over UTF-8 text. Kept deterministic and dependency-free for report integrity. */
export function sha256HexUtf8(payload: string): string {
  ...
  for (let offset = 0; offset < bytes.length; offset += 64) {
    ...
    hash[0] = add32(hash[0], a); hash[1] = add32(hash[1], b);
    hash[2] = add32(hash[2], c); hash[3] = add32(hash[3], d);
    hash[4] = add32(hash[4], e); hash[5] = add32(hash[5], f);
    hash[6] = add32(hash[6], g); hash[7] = add32(hash[7], h);
  }
